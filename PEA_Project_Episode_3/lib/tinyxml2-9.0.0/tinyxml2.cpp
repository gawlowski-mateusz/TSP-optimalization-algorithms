...
#   include <cstdarg>
	...
	static inline int TIXML_SNPRINTF( char* buffer, size_t matrix_size, const char* format, ... )
	{
		va_list va;
		va_start( va, format );
		const int result = vsnprintf_s( buffer, matrix_size, _TRUNCATE, format, va );
		va_end( va );
		return result;
	}

	static inline int TIXML_VSNPRINTF( char* buffer, size_t matrix_size, const char* format, va_list va )
	{
		const int result = vsnprintf_s( buffer, matrix_size, _TRUNCATE, format, va );
		return result;
	}
